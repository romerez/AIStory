...
- Before page images, the app draws one cast/style character sheet and reuses it as the primary identity anchor for every page. It is regenerated on Retry all pictures and can be redrawn on demand.
- It also draws a background/location sheet (an empty establishing plate with no characters) and reuses it as the setting anchor, which keeps backgrounds consistent across pages. It follows the same regenerate/retry rules as the character sheet.
- A self-hosted local image option is available: ComfyUI behind a small OpenAI-shaped proxy, with FLUX schnell, SDXL, and SDXL + IP-Adapter "consistent" presets (plus a hi-res RealESRGAN upscale variant). The character and background sheets feed IP-Adapter as identity/setting anchors. Settings shows live ComfyUI/proxy/IP-Adapter status.
- A per-book image seed (with a reroll control) makes local image generation reproducible; each page's seed is derived deterministically from it.
...
- A standalone in-app PDF library (the current export is the browser print dialog, now defaulting to A4 landscape with rendered page/background colors and a book-title filename).
- Generated or composed cover and end pages (planned; deferred for now).
...
- `src/data/persistentStorage.js` - IndexedDB-backed latest-book, story-history, and Step 1 setup-draft persistence.
...
- The current Step 1 setup draft is saved in IndexedDB under the `aistory-persistence` database, `setup-draft` key. The form autosaves as the user edits and also has a visible Save setup button.
...
type GeneratedBook = {
  ...
  storyTitle: string;
  storySummary: string;
  workflowStage?: "story-draft" | "image-building" | "complete";
  targetAge: string;
  language: string;
  theme: string;
  artStyle: string;
  characterSheetUrl?: string;
  characterSheetCreatedAt?: string;
  locationSheetUrl?: string;
  locationSheetCreatedAt?: string;
  imageSeed?: number;
  generatedStyleReferenceUrl?: string;
  visualBible: {
...
Prompt repetition improves consistency but does not guarantee it. The strongest implemented anchor is a generated character/style sheet (see below). Further production consistency can still use provider-specific reference-image inputs, image-to-image/edit APIs, and seed controls where available.

Character/style sheet (primary consistency anchor):

- Before page images start, the app generates one cast/style reference sheet through the active image model: each named character full-body, front view, neutral pose, on a plain background, in the chosen art style. This runs once per build via `POST /api/generate-character-sheet`.
- The sheet is compressed to a smaller JPEG client-side and stored on the book as `characterSheetUrl`. It is then attached to every page image request as the top-priority reference, decoupling identity from any single story page.
- `Retry all pictures` clears `characterSheetUrl` so a fresh sheet is drawn before the restarted queue. A `Regenerate character sheet` action redraws only the sheet.
- The sheet must never block the build: if sheet generation fails, it is logged and the book proceeds using the legacy page-1 anchors.
- Local demo mode draws a simple SVG sheet so the flow is testable without paid APIs.

Background / location sheet (setting anchor, fixes background drift):

- A character sheet locks identity but not the environment, so backgrounds used to drift page to page. To fix that, the app also draws one background/location sheet: a single empty establishing plate of the setting with no characters, consistent landmarks/props/lighting, in the chosen art style. This runs once per build via `POST /api/generate-location-sheet` and is stored on the book as `locationSheetUrl`.
- It is attached to every page image request right after the character sheet (its own reserved slot), so character identity and the setting are both anchored. It follows the same rules as the character sheet: cleared and redrawn on `Retry all pictures`, redrawable on demand, and never blocks the build if it fails. Local demo mode draws a simple background-only SVG.

Page prompt behavior:

- Each page sends the full, descriptive `illustrationPrompt` first (identity tokens front-loaded). Wording is only softened — and only then — if a provider rejects the prompt for safety/policy: tier 1 is the real prompt, tier 2 is the word-softened variant, tier 3 is a plain wholesome fallback. Do not run the softening pass on every request; it strips age/character/setting words and weakens consistency.

Current reference-image behavior:

- Gemini image calls attach available data-URL reference images as inline image parts. OpenAI image calls use `/v1/images/edits` with multipart image inputs when references are available, and `/v1/images/generations` otherwise.
- Reference priority per page: the generated character sheet first (its slot is reserved so multiple character references cannot crowd it out), then the background/location sheet, then saved per-character references, then the overall style reference. The legacy page-1 anchors (`generatedStyleReferenceUrl` and the first completed page) are only used when no sheet exists.
- In the normal page-by-page flow, full generated page image data URLs are stripped before `/api/generate-image` requests to prevent page 2+ payload bloat. The compact `characterSheetUrl` (or `generatedStyleReferenceUrl`) travels instead.
- Do not chain every page into the next as the only strategy. Sequential chaining accumulates mistakes; prefer the character sheet and stable character references as the main anchor.
- Keep reference inputs limited and compressed. Large data-URL references increase request size, provider cost, and latency.
...
- `GET /api/local-image-health` proxies the local ComfyUI image proxy's `/health` server-side (avoiding browser CORS) so Settings can show whether the proxy, ComfyUI, and IP-Adapter are up and which workflows exist.
...
- `POST /api/generate-character-sheet` draws the cast/style reference sheet for a book through the active image model and returns the book with `characterSheetUrl` set. Local image models return an SVG sheet.
- `POST /api/generate-location-sheet` draws the empty background/location plate (no characters) for a book through the active image model and returns the book with `locationSheetUrl` set. Local image models return an SVG plate.
...
- Google Gemini 2.5 Pro, Gemini 2.5 Flash, and Gemini 2.5 Flash-Lite: `https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent`
- xAI Grok 4.3: `https://api.x.ai/v1/chat/completions`

Local / self-hosted story models:

- Any OpenAI-compatible local server (Ollama, LM Studio, llama.cpp, vLLM) works as a story model with `type: openai-compatible` and the server's chat-completions URL. Built-in preset `DictaLM 3.0 24B (local)` targets Ollama: endpoint `http://localhost:11434/v1/chat/completions`, model `hf.co/dicta-il/DictaLM-3.0-24B-Thinking-GGUF:Q4_K_M`. DictaLM is Hebrew-native, so it pairs well with the Hebrew + niqqud path.
- Local endpoints (localhost / 127.0.0.1 / `*.local`) need no API key — the `Authorization` header is omitted automatically.
- Local endpoints get a longer timeout (`AISTORY_LOCAL_PROVIDER_TIMEOUT_MS`, default 600000ms) than cloud calls (`AISTORY_PROVIDER_TIMEOUT_MS`, default 120000ms), since large quantized models can be slow, especially loading cold.
- "Thinking" models that emit `<think>...</think>` are handled: the story JSON parser strips think/thinking blocks and falls back to the outermost `{...}` object.
- Setup: install Ollama, run `ollama pull hf.co/dicta-il/DictaLM-3.0-24B-Thinking-GGUF:Q4_K_M`, then in Settings pick `DictaLM 3.0 24B (local)` and Use for stories. Local image (e.g. ComfyUI behind a `custom-image` endpoint) is the planned next local piece.

For Gemini story models, keep the static picker conservative. Model names can be unavailable for a specific key, region, or API version even when docs/search mention them. If a Gemini story model returns a 404 saying it is not found or not supported for `generateContent`, the backend calls Google's `models.list` endpoint and retries with an available `generateContent` model, preferring Gemini 2.5 Flash.
...
Local / self-hosted image models (ComfyUI behind the local image proxy at `http://127.0.0.1:8989/v1/images/generations`, keyless):

- `ComfyUI FLUX schnell (fast)` → workflow `flux-storybook`.
- `ComfyUI SDXL` → workflow `sdxl-storybook` (negative prompt enabled).
- `ComfyUI SDXL Consistent` → workflow `sdxl-ipadapter-storybook` (IP-Adapter references, up to 2: character sheet + background sheet; negative prompt enabled).
- `ComfyUI SDXL Consistent (Hi-Res)` → workflow `sdxl-ipadapter-storybook-hires` (same graph, then RealESRGAN x4 + downscale for ~1.5x sharper output).

These are `type: custom-image` profiles with per-profile capability flags, so the generic/cloud `custom-image` contract stays byte-identical when the flags are off: `supportsNegativePrompt`, `supportsReferences`, `referenceLimit`, and optional `imageWidth`/`imageHeight`. The proxy bridges AIStory's OpenAI-shaped `custom-image` request to ComfyUI's graph API and uploads reference images; the shipped default active image model stays cloud, so a user opts into ComfyUI by selecting a preset in Settings. See `local-stack/README.md`.
...
- `custom-image` sends a generic JSON `{ model, prompt, input }` request to the configured endpoint and extracts common image response fields. When the active profile sets the matching capability flags, the request also includes `negative_prompt` (`supportsNegativePrompt`), `reference_images` (`supportsReferences`, capped at `referenceLimit`), a deterministic per-page `seed` derived from `book.imageSeed + pageNumber`, and `width`/`height` (`imageWidth`/`imageHeight`). With no flags set the body is unchanged, so strict OpenAI-shaped servers are unaffected.
...
- PDF saving uses the browser print dialog with A4 landscape print CSS that now forces page/background colors to render and defaults the saved file name to the book title. There is no dedicated PDF generation library yet.

## 13. Recommended Next Steps

1. Remove browser-stored provider keys once server env/key management is enough for normal testing.
2. Done: character/style and background/location sheets are generated before page images and pinned as cross-page anchors; a per-book image seed control is available; local ComfyUI image generation (SDXL/FLUX, SDXL + IP-Adapter consistency, optional hi-res RealESRGAN upscale) is wired through the local image proxy. Next: per-character sheets and ControlNet pose/depth where providers support them.
...
6. Add richer final book layouts, such as a cover page, end page, and optional full-bleed image pages (either composed from existing art with overlaid text, or generated through the active image model using the character/background sheets as references).
...
17. Generate book images and confirm a "Preparing the character sheet" state appears first, then the per-image progress, and Steps mode moves to Step 3.
...
28. Use Retry all pictures and confirm a fresh character sheet is drawn, then every image is regenerated from page 1 with progress shown.
